...
**Progress:** T1 ✅ done · A1 ✅ done (log survives a plan rebuild) · A2 ✅ done (17→62 exercises, one catalogue, equipment filter) · A3 ✅ done (five questions; sessions built to the time you have) · A4 ✅ done (swap any exercise; it sticks) · A5 ✅ done (guided player: supersets alternate, one-tap logging, holds count down, and a rest timer measured against the clock so it stays right when the phone is backgrounded — measured 20 s away, 0 s drift) · A6 ✅ done (estimated 1RM, this week's volume and a trend arrow on every lift; the kg box suggests a next load as a placeholder — never written into a row as if typed — and says why; a toast on a genuine new best) · A7 ✅ done (4–12 week programmes: blocks that progress with an ease-off every fourth week, an A/B(/C) rotation, sessions placed on days that do not fight the running and movable by hand, and the plan's own strength steps aside with a note saying so) · A8 ✅ done (a finished strength session reaches Strava as Weight Training — never a Run — using the exact same connection and auto-send switch a run already uses; a handshake in `/strava/status` refuses to send until the deployed Worker has confirmed it understands the sport type, so deploy skew fails closed) · D3a ✅ done (the wizard opens on one health question; four answers stop a plan being built at all, two require an explicit “I understand, continue”, and nothing you tick is stored anywhere — which is what makes the promise on that screen true) · **D3b IS REPLACED BY TRACK Y — a tailored 12-17 programme, your instruction of 21 Sept, not a gate.** Research done and committed as `YOUTH.md`; the double-check overturned three things, including the table you first supplied (it is the 1987 IAAF folklore set and every row exceeds UKA's own rule). Your rulings: **UKA's competition rule** as the basis, **18 is an adult**, and your frequency column adopted unchanged. Y0 ✅ research · Y1 ✅ done (age asked before the goal; only goals that age may race are offered, enforced at the picker AND at save) · Y2 ✅ done (the ceilings reach the plan itself: long run, weekly volume, running days, one key day a week, and no goal-pace rehearsal under 15 — 16 and 17 fully inside both limits; two structural residuals recorded in YOUTH.md 5.7) · Y3 ✅ done (weight training by age — offered at every age, because both position stands say there is no minimum and the growth-plate fear is a myth; what changes is HOW. A 12-17 session is prescribed by a rep range and never by a percentage of a one-rep max, because naming one tells a 14-year-old to go and test theirs and that is the single thing both sources forbid outright. Sets capped at 3, jumps at 6 reps a set, 3 sessions a week, and a supervision line on every session saying the app can tell you what to do and cannot watch you do it. The estimated 1RM is withheld on screen — the trend arrow is not. **0 of 72 adult plans changed**; the one adult-visible difference is 40 of 144 programme descriptions that were overstating their own set count and now do not) · Y4 ✅ done (Strava's own age rules, verified on Strava's pages: nothing under 13, no heart rate under 16. Found under it: the app never handed the age to the plan builder, so Y2/Y3 reached no real plan until now — a 13-year-old was being given sessions up to 14 km and five runs a week — and "Prefer not to say" was stored as 0 and read as a 12-year-old. Both fixed; 20 of 20 re-breaks caught) · **next: Y5** Children's Code, Y6 App Store answers. Then D3c (a home for the wellbeing tick-boxes).
...
| T2–T6 tooling | Sonnet | D3a ✅ · **Y0–Y6 youth** · D3c | **Opus** | B8 preference dials | Opus |
...
**D3a · Safety before the plan** (`pc-safety`) — ✅ **DONE 2026-09-21.** See CLAUDE.md's dated chapter for what was measured; the two things it deliberately does NOT do are gate the old profile setup form and gate STARTING a run. Original spec: new wizard step `safety` first in both branches of `wizStepIds()` (`:22720`): `checks(FLAGS_PHYS)` + "None of these", `checkinConsent()` (the consent guard demands it), `EMERGENCY_BANNER()`. On Next: `RC.screenRedFlags`; emergency → "Get help first", plan not built; urgent/professional → guidance + explicit "I understand, continue"; else proceed. Tests: `safety` precedes `goal`; `wizardFinish` unreachable while blocked; onboarding tests green in three timezones.
