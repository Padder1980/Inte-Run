...
| `WorkoutView.swift` | The five live pages, plus the finish summary. |
| `MetricsPage.swift` | The live numbers page, as a **pure view** (plain strings in) so `WatchPreview` can render it. |
| `PacePage.swift` | The pace page: step countdown, the prescribed band with a current-pace bubble, lap pace. Pure view, same reason. ⚠️ Axis is slow-LEFT/fast-RIGHT and labels/zone/marker must all agree — its predecessor (`PaceBandView.swift`, deleted) printed labels one way and computed the marker the other, so a slow runner saw the marker at the fast end. |
| `PaceView.swift` / the `metrics` var in `WorkoutView` | Thin mappers from `WorkoutManager` onto the two pure pages. |
| `WatchPreview.swift` | DEBUG-only: `-InteRunWatchPreview <scene>` launch argument renders any run-screen state without a workout — simctl can screenshot a watch but cannot tap it. Never reaches Release (verified by `strings` on the binary). |
...
### The live run: five pages

Laid out the way a runner's thumb already knows from Apple's Workout app —
**Controls ← Metrics → Pace → Session → Music** — landing on Metrics, because that is the page you
glance at and the controls being one swipe away is what stops a stray touch ending your run. The
pace band lives on its own page (one swipe right of Metrics), full size, in the owner's chosen
reference layout: step countdown as the hero, the band with a bubble saying the current pace, lap
pace beneath.
