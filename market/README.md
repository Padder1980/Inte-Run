...
It lives in this repo but shares nothing with Inte-Run: separate folder, separate build, separate
storage keys. Nothing here touches the running app.
...
```bash
npm ci                        # once
node market/build.ts          # writes market/index.html
```

Then open `market/index.html` in a browser. It is one self-contained file — no server, no build step
...
node --test "market/test/*.test.ts"   # 35 engine tests
node market/test/app-smoke.mjs        # 16 browser checks against the BUILT page
```
...
```
market/
  engine/        pure TypeScript, no runtime dependencies, fully unit-tested
...
**⚠️ Never hand-edit `market/index.html`** — it is generated. Edit `shell.html` or the engine and
re-run `node market/build.ts`.

**The page template is a separate file on purpose.** Inte-Run's `web/app.ts` holds its entire runtime
inside one giant JS template literal, which means no backtick may appear anywhere in the app's own
code — not even in a comment — on pain of a silent build failure. That rule has fired at least six
times in this repo's history, twice hiding behind a stale build that still passed its checks. Here
the HTML lives in `shell.html`, so the app's JavaScript is just JavaScript and there is nothing to
escape.
