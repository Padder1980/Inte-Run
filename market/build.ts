// Builds market/index.html — one self-contained page, no external requests for code or styling.
//
// ⚠️ THE PAGE TEMPLATE IS A SEPARATE FILE (`shell.html`), NOT A TEMPLATE LITERAL IN HERE, AND THAT
// IS DELIBERATE. The running app's `web/app.ts` holds its entire runtime inside one giant JS
// template literal, which means no backtick may appear anywhere in the app's own code — not even in
// a comment — on pain of a silent build failure. That rule has fired at least six times in this
// repo's history, twice hiding behind a stale build that still passed its checks. Keeping the HTML
