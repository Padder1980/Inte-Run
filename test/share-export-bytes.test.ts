...
/**
 * ⚠️ THE WANTED SIZE IS A TABLE HERE AND A TERNARY WOULD BE WRONG NOW. Written as "feed or story", a
 * square export was measured against 1080x1920 and would have had to be 1920 tall to pass — so the
 * guard would have failed loudly, which is the good case. Written the other way round ("story or not
 * story") every non-story shape would be checked against the feed's 1350 and a 1080-tall square would
 * fail for the same reason. The table cannot express either mistake.
 */
const WANT: Record<string, [number, number]> = { story: STORY, feed: FEED, square: SQUARE };
...
test("BLOCKER: Story is exactly 1080x1920, Feed 1080x1350 and Square 1080x1080, in the BYTES", () => {
  const bad: string[] = [];
  ...
  // ⚠️ THREE ASPECTS TIMES TWO FIT MODES. The owner's photo-fit ruling made two pictures out of one and
  // the square makes three out of two; a gate that measured only the default would leave the opt-in
  // cover crop — which every runner with a panorama will reach for — with no proof of its own
  // dimensions at all, and one that counted a fixed sixteen would silently stop covering a shape.
...
test("BLOCKER: all four templates reach all three aspects, and no card silently fell back to another", () => {
  // ⚠️ A DIMENSION GUARD PASSES WHATEVER WAS DRAWN. Sixteen correctly sized pictures of the wrong
...
test("BLOCKER: every export is a real JPEG (the poster may be PNG) and carries a complete frame", () => {
  for (const s of shots()) {
    const kind = imageKind(s.bytes);
    if (s.drawn === "route") {
      // The spec: "Graphic Route Poster: PNG is acceptable when file size remains practical." So
      // either is allowed here and nowhere else.
      assert.ok(kind === "jpeg" || kind === "png", s.key + " is neither JPEG nor PNG: " + kind);
    } else {
      assert.equal(kind, "jpeg", s.key + " is a " + kind + ", and a photo template must be JPEG");
    }
    assert.equal(s.type, "image/jpeg", s.key + ": the File's declared type is " + s.type);
    if (kind !== "jpeg") continue;
...
test("BLOCKER: the Feed and Square exports are recompositions, not crops or scales of the Story", () => {
  ...
  //
  // ⚠️ THE SQUARE IS THE SHAPE THIS TEST MATTERS MOST FOR, because the lazy square is exactly a centre
  // crop of a story — it needs no new type scale, no metric cap and no shorter ladder, and it looks
  // plausible until you notice the wordmark has gone. The contract's words are "Recompose to a compact
  // poster" and "do not merely crop or uniformly scale the 9:16 design".
  const rows: string[] = [];
  for (const t of ["moment", "execution", "progression", "route"]) {
...
test("the missing-data states are distinct where they should be, and identical where that is correct", () => {
  ...
  assert.deepEqual(collisions.sort(),
    ["no-target/feed = no-verdict/feed",
      ...
      "no-target/square = no-verdict/square",
      "no-target/square/fill = no-verdict/square/fill",
      "no-target/story = no-verdict/story",
      "no-target/story/fill = no-verdict/story/fill",
      "route/feed = route/feed/fill",
      "route/square = route/square/fill",
      "route/story = route/story/fill"],
