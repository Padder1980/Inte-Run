...
/**
 * ⚠️ THERE IS DELIBERATELY NO PIXEL GUARD HERE FOR "NO SCRIM", AND THE REASON IS WORTH RECORDING.
 *
 * The obvious one — the lower half of the card must not be systematically darker than the upper — is
 * confounded by the fixture. This gate's source photograph is a sky-to-ground gradient (#a6c9ea to
 * #242c20 by construction), so its own lower half measures 0.599 of its upper before anything is drawn
 * over it. A bar loose enough to pass that is far too loose to catch a real wash, and a bar tight enough
 * to catch a wash fails on the photograph. Separating the two needs a photograph-only baseline shot for
 * every case, which is a capture this gate does not take.
 *
 * So the invariant lives in test/share-render.test.ts, where it can be EXACT: the renderer's draw calls
 * are recorded, and the claim is that no gradient and no near-full-width translucent fill is constructed
 * over the picture at all. A draw call that never happens cannot be confounded by a fixture.
 *
 * ⚠️ DO NOT "RESTORE" A PIXEL VERSION OF THIS WITHOUT THE BASELINE. It will pass on this fixture whatever
 * the renderer does.
 */
