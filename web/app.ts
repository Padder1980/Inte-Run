...
function buildOverviewMap(container, route) {
  ...
  loadRouteMap(route, OVMAP_W, OVMAP_H).then((md) => {
    ...
    g.fillStyle = "#0a1714"; g.fillRect(0, 0, OVMAP_W, OVMAP_H);
    md.tiles.forEach((t) => { try { g.drawImage(t.img, t.dx, t.dy, MAP_TILE, MAP_TILE); } catch (e) {} });
    g.fillStyle = "rgba(6,20,17,.28)"; g.fillRect(0, 0, OVMAP_W, OVMAP_H); // gentle dim for route contrast
    const overlay = routeMapSvg(route, md.proj, OVMAP_W, OVMAP_H);
    container.innerHTML = "";
    container.appendChild(cv);
    container.appendChild(el('<div class="ov-mapov">' + overlay + '</div>'));
    container.appendChild(el('<div class="ov-attr">© OpenStreetMap © CARTO</div>'));
  }).catch(() => {});
}
