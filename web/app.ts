...
function loadRouteMap(route) {
  return new Promise((resolve, reject) => {
    if (!route || route.length < 2) return reject(new Error("no route"));
    const lats = route.map((p) => p.lat), lngs = route.map((p) => p.lng);
    const minLa = Math.min(...lats), maxLa = Math.max(...lats), minLo = Math.min(...lngs), maxLo = Math.max(...lngs);
    const pad = 90; let z = 18;
    for (; z >= 3; z--) { if (mercX(maxLo, z) - mercX(minLo, z) <= MAP_W - 2 * pad && mercY(minLa, z) - mercY(maxLa, z) <= MAP_H - 2 * pad) break; }
    const originX = mercX((minLo + maxLo) / 2, z) - MAP_W / 2, originY = mercY((minLa + maxLa) / 2, z) - MAP_H / 2;
    const n = Math.pow(2, z), specs = [];
    for (let tx = Math.floor(originX / MAP_TILE); tx <= Math.floor((originX + MAP_W) / MAP_TILE); tx++)
      for (let ty = Math.floor(originY / MAP_TILE); ty <= Math.floor((originY + MAP_W) / MAP_TILE) && ty <= Math.floor((originY + MAP_H) / MAP_TILE); ty++) {
        if (ty < 0 || ty >= n) continue;
        const wx = ((tx % n) + n) % n, sub = ["a", "b", "c"][((tx % 3) + 3) % 3];
        specs.push({ url: "https://" + sub + ".basemaps.cartocdn.com/dark_all/" + z + "/" + wx + "/" + ty + "@2x.png", dx: tx * MAP_TILE - originX, dy: ty * MAP_TILE - originY });
      }
...
function buildShareCanvasWithMap(run) { return loadRouteMap(run.route).then((md) => buildShareCanvasCore(run, md)); }
