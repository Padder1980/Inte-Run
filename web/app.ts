...
function applyProfile(pf) {
  ...
  return { ath, goal, plan, fitness, classification: cls, masters };
}

let profile = loadProfile() || Object.assign({}, DEFAULT_PROFILE);
let PLAN, FITNESS, CLASS, MASTERS;
function recompute() { const r = applyProfile(profile); PLAN = r.plan; FITNESS = r.fitness; CLASS = r.classification; MASTERS = r.masters; }
try { recompute(); } catch (e) { profile = Object.assign({}, DEFAULT_PROFILE); recompute(); }

const state = { tab: "today", screen: null, dayType: "quality", subj: { soreness: "none", energy: "good", stress: "low", motivation: "high", illness: "none" }, planWeek: PLAN.defaultWeekIndex, actTab: "performance", support: null };
...
function wire() {
  ...
  const save = $("saveProfile"); if (save) save.onclick = () => {
    ...
    profile = pf; PLAN = out.plan; FITNESS = out.fitness; CLASS = out.classification; MASTERS = out.masters; state.planWeek = PLAN.defaultWeekIndex; saveProfileStore();
...
function buildNav() {
  ...
  document.querySelectorAll(".navbtn").forEach((b) => b.onclick = () => { state.screen = null; state.tab = b.dataset.tab; if (b.dataset.tab !== "support") state.support = null; render(); });
}
