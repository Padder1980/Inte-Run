...
function draftFromForm() {
  ...
  // noRecent and seed a gentle beginner baseline (5 km at ~7:00/km) so a starter plan can be built.
  const recRaw = $("s_rectime").value.trim();
  let noRecent = false, recentDistM, recentTimeS;
  if (!recRaw) {
    noRecent = true; recentDistM = 5000; recentTimeS = 2100;
  } else {
...
// ---- 1 km time-trial recorder --------------------------------------------
// Lets a runner who hasn't done a 1 km test record one now: a live stopwatch whose result drops
// straight into the 1 km field. The overlay lives outside #view so the setup form keeps its values.
let TRIAL = null;
...
function trialTick() { if (!TRIAL || !TRIAL.running) return; $("trialClock").textContent = fmtClock(performance.now() - TRIAL.start); TRIAL.raf = requestAnimationFrame(trialTick); }
function ensureTrial() {
  if ($("trialOv")) return;
  const html = '<div class="trial-ov" id="trialOv"><div class="trial-panel">' +
    '<button class="trial-x" id="trialClose" aria-label="Close">✕</button>' +
    '<div class="trial-eyebrow">1 km time trial</div>' +
    '<div class="trial-clock num" id="trialClock">0:00.0</div>' +
    '<div class="trial-msg" id="trialMsg"></div>' +
    '<div class="trial-controls" id="trialCtrls"></div>' +
    '<div class="trial-foot">On a phone with GPS the full app auto-detects your 1 km and stops for you. Here you time it yourself — 2½ laps of a 400 m track is 1 km.</div>' +
    '</div></div>';
  document.querySelector(".app").appendChild(el(html));
  $("trialClose").onclick = closeTrial;
  $("trialOv").onclick = (e) => { if (e.target === $("trialOv")) closeTrial(); };
}
function openTrial() { ensureTrial(); TRIAL = { running: false, start: 0, secs: 0, raf: null }; trialReady(); $("trialOv").classList.add("on"); }
function closeTrial() { if (TRIAL && TRIAL.raf) cancelAnimationFrame(TRIAL.raf); if ($("trialOv")) $("trialOv").classList.remove("on"); TRIAL = null; }
function trialReady() {
  $("trialClock").textContent = "0:00.0";
  $("trialMsg").innerHTML = "Warm up first. Press <b>Start</b> and run 1 km as hard as you can hold — tap <b>Finish</b> the moment you complete it.";
  const c = $("trialCtrls"); c.className = "trial-controls"; c.innerHTML = '<button class="primary" id="trialStart">Start</button>';
  $("trialStart").onclick = trialStart;
}
function trialStart() {
  TRIAL.running = true; TRIAL.start = performance.now(); TRIAL.raf = requestAnimationFrame(trialTick);
  $("trialMsg").innerHTML = "<b>Go!</b> Hold your hardest sustainable effort. Tap <b>Finish</b> at exactly 1 km.";
  const c = $("trialCtrls"); c.className = "trial-controls two";
  c.innerHTML = '<button class="ctrl" id="trialCancel">Cancel</button><button class="primary" id="trialFinish">Finish</button>';
  $("trialCancel").onclick = trialResetReady; $("trialFinish").onclick = trialFinish;
}
function trialResetReady() { if (TRIAL.raf) cancelAnimationFrame(TRIAL.raf); TRIAL.running = false; trialReady(); }
function trialFinish() {
  if (TRIAL.raf) cancelAnimationFrame(TRIAL.raf);
  TRIAL.running = false;
  const secs = Math.round((performance.now() - TRIAL.start) / 1000);
  TRIAL.secs = secs;
  $("trialClock").textContent = fmtTimeFull(secs);
  const c = $("trialCtrls"); c.className = "trial-controls two";
  if (secs >= 150 && secs <= 480) {
    const m = RC.computeMas(secs);
    $("trialMsg").innerHTML = "That's <b>" + fmtTimeFull(secs) + "</b> for 1 km — MAS <b>" + m.masMps.toFixed(2) + " m/s</b>. This will set your VO₂/interval paces.";
    c.innerHTML = '<button class="ctrl" id="trialRedo">Redo</button><button class="primary" id="trialUse">Use this time</button>';
    $("trialUse").onclick = trialUse;
  } else {
    $("trialMsg").innerHTML = secs < 150
      ? "That's under 2:30 — too fast for a full 1 km, so it can't set MAS. Check the distance and try again."
      : "That's over 8:00 — for MAS we need a hard 1 km effort. Have a rest and try again.";
    c.className = "trial-controls"; c.innerHTML = '<button class="primary" id="trialRedo">Try again</button>';
  }
